...
**Returns:**
- `{ screenshotId, virtualPath, createdAt, mimeType, bytes, width?, height?, path }`
- `virtualPath` format: `screenshot://<screenshotId>`
...
4. **Storage:**
   - Screenshot metadata includes a `virtualPath` (`screenshot://...`) for downstream resolution
   - Prefer storing metadata references instead of embedding large image payloads in chat content
